...
namespace Achievability
{
    class Program
	{
        static void Main(string[] args) {
			...
			var T7 = new Goal("T7", GoalTypes.TASK, DecompositionTypes.AND, new List<Goal>(), new List<WorldContext>() { new TecnologyAversionContext(DecompositionTypes.AND, false) });
			var T8 = new Goal("T8", GoalTypes.TASK, DecompositionTypes.AND, new List<Goal>(), new List<WorldContext>() { new TecnologyAversionContext(DecompositionTypes.AND, false) });
			var T9 = new Goal("T9", GoalTypes.TASK, DecompositionTypes.OR, new List<Goal>(), new List<WorldContext>() { new TecnologyAversionContext(DecompositionTypes.OR, false), new HomeAssistanceContext(DecompositionTypes.OR) });
			var T10 = new Goal("T10", GoalTypes.TASK, DecompositionTypes.OR, new List<Goal>(), new List<WorldContext>() { new TecnologyAversionContext(DecompositionTypes.OR, false), new HomeAssistanceContext(DecompositionTypes.OR) });
			var T11 = new Goal("T11", GoalTypes.TASK, DecompositionTypes.OR, new List<Goal>(), new List<WorldContext>() { new HomeAssistanceContext(DecompositionTypes.AND), new TecnologyAversionContext(DecompositionTypes.AND, false) });
			...
			var G16 = new Goal("G16", GoalTypes.GOAL, DecompositionTypes.OR, new List<Goal>() { T15 }, new List<WorldContext>() { new HomeAssistanceContext(DecompositionTypes.AND), new TecnologyAversionContext(DecompositionTypes.AND, false) });
			...
			var G10 = new Goal("G10", GoalTypes.GOAL, DecompositionTypes.AND, new List<Goal>() { T21 }, new List<WorldContext>() { new HealthRiskContext(DecompositionTypes.AND) });
			...
			Context mary = new Context(new List<FactTypes>() { FactTypes.HasDiabetes, FactTypes.ProneToFalling, FactTypes.HasOsteoporosis, FactTypes.CanWalk, FactTypes.HasAnAssistedLivingDevice });

			Console.WriteLine("Start");

			var plan = isAchievable(cgm, mary);
		...
		public static Plan isAchievable(CGM cgm, Context currentPersona) {
			Goal root = cgm.getRoot();
